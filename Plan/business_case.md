...
```mermaid
graph TD
    A[Rủi ro Vận hành] --> B(Lệch tỷ lệ Nam/Nữ)
    A --> C(Khách bùng lịch No-show)
    A --> D(Rủi ro Danh tiếng/An toàn)
    
    B --> B1[Xây dựng danh sách dự phòng - Waiting List]
    C --> C1[Thu tiền vé trước 100% để giữ chỗ]
    D --> D1[Yêu cầu ký cam kết bảo mật & xác thực CCCD]
```
